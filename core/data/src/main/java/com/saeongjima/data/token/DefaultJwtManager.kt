...
class DefaultJwtManager @Inject constructor(private val dataStore: DataStore<Preferences>) :
    JwtManager {
    ...
    override suspend fun saveRefreshJwt(token: String) {
        dataStore.edit { preferences ->
            preferences[refreshJwtKey] = token
        }

        dataStore.data.map { preferences ->
            preferences[refreshJwtKey]
        }.firstOrNull()
    }
    ...
    override suspend fun clearAllTokens() {
        dataStore.edit { preferences ->
            ...
            preferences.remove(refreshJwtKey)
        }
    }
